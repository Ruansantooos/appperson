...
export const Card: React.FC<{ children: React.ReactNode; className?: string; variant?: 'dark' | 'peach' | 'blue' | 'orange' }> = ({ children, className, variant = 'dark' }) => {
  const variants = {
    dark: "bg-[#161616] border border-white/5",
...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({ className, ...props }) => (
  <input 
    className={`w-full px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:ring-2 focus:ring-[#c1ff72] focus:outline-none transition-all ${className}`}
    {...props}
  />
);

// Extended Badge variants to support all usage cases (info, danger, warning)
export const Badge: React.FC<{ children: React.ReactNode; variant?: 'success' | 'status' | 'default' | 'info' | 'danger' | 'warning' }> = ({ children, variant = 'default' }) => {
  const variants = {
  ...
  return (
    <span className={`px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${variants[variant]}`}>
      {children}
    </span>
  );
};
